    ...
    if (json->get(d, "isArmed") || json->get(d, "security/isArmed")) {
      ...
      if (!isArmed) {
        ...
        Firebase.RTDB.setBoolAsync(
            &fbTele, ("devices/" + deviceId + "/security/alarmActive").c_str(),
            false);
      }
    }
...
void setup() {
  ...
  ArduinoOTA.onStart([]() {
    isOTAActive = true;
// 🛡️ DE-INITIALIZE WATCHDOG FOR FLASH SAFETY
    ...
    if (forceTelemetry || (millis() - lastTelemetryTime > reportInterval)) {
      if (Firebase.ready() && isInternetLive) {
        ...
        if (Firebase.RTDB.updateNodeAsync(&fbTele, pathTele.c_str(), &j)) {
          ...
          // 💓 Vitality Heartbeat & Alarm Cleanup
          FirebaseJson status;
          status.set("online", true);
          status.set("lastSeen", (int)time(NULL));
          status.set("rssi", (int)WiFi.RSSI());
          status.set("ldrValid", true);
          Firebase.RTDB.updateNodeAsync(
              &fbTele, ("devices/" + deviceId + "/status").c_str(), &status);
