...
<p align="center">
  <a href="#">
    <img src="https://readme-typing-svg.demolab.com?font=Orbitron&weight=900&size=26&pause=1000&color=00FFFF&center=true&vCenter=true&width=800&lines=Evolving+from+Local+Mesh+to+Cloud+Supremacy;Industrial-Grade+Reliability;Zero-Latency+Achieved;The+In-App+Maker+Ecosystem" alt="Typing SVG" />
  </a>
</p>
...
### 🧠 The Logic of Human Detection (PIR Calibration)
Raw analog PIR sensors are notoriously noisy. Electromagnetic interference (EMI) from the active 220V relays jumping, simple thermal drafts from air conditioners, and main-line voltage spikes all create phantom ghost triggers. Our newly engineered firmware filters noise by treating raw analog triggers as "suspicions," not absolute facts. 

**The Algorithm (Pulse Validation Array):**
...
### 1. 🛡️ Industrial Resilience & Watchdog Eviction
The core firmware no longer uses `delay()`. We aggressively re-wrote the codebase to an asynchronous, `millis()`-driven relay pipeline, completely neutralizing the 15-second Core-0 RTOS watchdog. The `softRecoveryEngine` now strictly enforces a 30-second cooldown block instead of infinite looping.

### 2. 📡 Absolute Telemetry Truth
1. Command is fired natively via the App.
2. The UI instantly responds and artificially "pins" the active toggle visual state for 5 seconds.
3. The ESP32 receives the trigger, fires the hardware relay, and strictly *mutes* its Firebase telemetry pushes for 2 seconds to permanently prevent visually bouncing toggle switches inside the Flutter App.

### 3. 🌟 The In-App Maker Ecosystem
...
* 💾 **Direct Hardware Source Code:** Instantly copy/download the raw **ESP32 Hub** and **ESP8266 Satellite** C++ templates securely decoupled from GitHub.
